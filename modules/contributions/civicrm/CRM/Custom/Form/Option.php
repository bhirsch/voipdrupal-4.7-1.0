...
class CRM_Custom_Form_Option extends CRM_Core_Form {
     ...
     function postProcess()
    {
        ...
        if ( $customField->find( true ) && ($customField->html_type == 'CheckBox' || $customField->html_type == 'Multi-Select')) {
            $defVal = explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, $customField->default_value);
            ...
            if ( CRM_Utils_Array::value( 'default_value', $params ) ) {
                if ( !in_array($customOption->value, $defVal) ) {
                    ...
                    if ( empty($defVal[0]) ) {
                        ...
                        $customField->default_value .= CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR.$customOption->value;                   
                    }
                ...
                $customField->default_value = implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, $tempVal);                
                $customField->save(); 
            }           
        } else {            
            if ( CRM_Utils_Array::value( 'default_value', $params ) ) {
