...
class CRM_Core_BAO_CustomGroup extends CRM_Core_DAO_CustomGroup {
     ...
     *
     */
      function &getTree($entityType, $entityId=null, $groupId=0)
    {
        // create a new tree
        $groupTree = array();
     ...
     *
     */
      function getGroupDetail($groupId = null, $searchable = null, $extends = null)
    {
        // create a new tree
        $groupTree = array();
     ...
     *
     */
      function _getTableName($entityType)
    {
        $tableName = '';
        switch($entityType) {
        case 'Contact':
     ...
     *
     */
      function _addWhereAdd(&$customGroupDAO, $entityType)
    {
        switch($entityType) {
        case 'Contact':
    ...
    { 
     ...
     function setDefaults( &$groupTree, &$defaults, $viewMode, $inactiveNeeded ) {
        foreach ( $groupTree as $group ) {
            $groupId = $group['id'];
            foreach ($group['fields'] as $field) {
                ...
                case 'CheckBox':
                    if ($viewMode) {
                        ...
                        if (isset($field['customValue']['data'])) {
                            $checkedData = explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, $field['customValue']['data']);
                            foreach($customOption as $val) {
                                if (in_array($val['value'], $checkedData)) {
                                    $defaults[$elementName][$val['value']] = 1;
                                } else {
     ...
     function postProcess( &$groupTree, &$params ) {
        ...
        foreach ($groupTree as $group) {
            foreach ($group['fields'] as $field) {
                ...
                case 'CheckBox':  
                    ...
                    if ( ! empty( $v ) ) {
                        ...
                        $groupTree[$groupId]['fields'][$fieldId]['customValue']['data'] = 
                            implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, $customValue);
                    } else {
                        $groupTree[$groupId]['fields'][$fieldId]['customValue']['data'] = null;
                    }
                    break;
                ...
                case 'Multi-Select':  
                    ...
                    if ( ! empty( $v ) ) {
                        $groupTree[$groupId]['fields'][$fieldId]['customValue']['data'] = 
                            implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR, $v);
                    } else {
                        $groupTree[$groupId]['fields'][$fieldId]['customValue']['data'] = null;
                    }
                    break;
     ...
     * @static
     */
     function buildViewHTML( &$page, &$groupTree,
                                   $viewName = 'viewForm',
