...
class CRM_Core_BAO_ACL extends CRM_Core_DAO_ACL {
     ...
     * @static
     */
      function permissionClause(&$tables, $operation,
                ...
                $acl_id = null, $acl_group = false) 
    {
        ...
        while ($dao->fetch()) {
            ...
            else 
            {
                ...
                $clause = CRM_Core_BAO_ACL::getClause(
     ...
     * @static
     */
      function getClause($table, $id, &$tables) {
     ...
     * @static
     */
      function &getGroupACLs($contact_id, $aclGroups = false) {
        $contact_id = CRM_Utils_Type::escape($contact_id, 'Integer');
        $group_id   = CRM_Utils_Type::escape($group_id, 'Integer');
        ...
        if ($aclGroups) {
            $results += CRM_Core_BAO_ACL::getGroupACLGroups($contact_id);
        }
    ...
    */
     ...
     * @static
     */
      function &getAllByContact($contact_id) {
        $result = array();

        /* First, the contact-specific ACLs, including ACL Groups */
        $result += CRM_Core_BAO_ACL::getACLs($contact_id, null, true);

        /* Then, all ACLs granted through group membership */
        $result += CRM_Core_BAO_ACL::getGroupACLs($contact_id, true);

        /* Finally, get the domain-level ACL rules */
        $result += CRM_Core_BAO_ACL::getACLs(null, null, true);
