...
$GLOBALS['_CRM_CORE_BAO_ADDRESS']['_overwrite'] =  true;
...
class CRM_Core_BAO_Address extends CRM_Core_DAO_Address {
     ...
     function dataExists(&$params, $locationId, &$ids)
    {
        ...
        if ( $GLOBALS['_CRM_CORE_BAO_ADDRESS']['_overwrite'] && is_array( $ids ) && CRM_Utils_Array::value('address', $ids['location'][$locationId]) ) {
            return true;
        }
     ...
     */
     ...
     function setOverwrite( $overwrite ) {
        $GLOBALS['_CRM_CORE_BAO_ADDRESS']['_overwrite'] = $overwrite;
    }
