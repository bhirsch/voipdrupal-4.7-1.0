...
class CRM_Core_BAO_CustomOption extends CRM_Core_DAO_CustomOption {
     ...
     * @static
     */
     function getCustomOption($fieldId, $inactiveNeeded=false)
    {       
     ...
     * @access public
     */
     function getCustomValues($fieldId)
    {
        ...
        $customValue = array();
        $customValue = explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR ,$values);
        return $customValue;
    }

     function getOptionLabel( $fieldId, $value, $fieldType = null ) {
        if ( $fieldType &&
             $fieldType != 'Select' &&
             $fieldType != 'Radio' ) {
            return $value;
     ...
     function del($optionId) 
    {
        ...
        if( $fieldDAO->html_type !='CheckBox' && $fieldDAO->html_type !='Multi-Select' ) {
            ...
            while($customValueDAO->fetch()) {
                ...
                $customValue = explode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR ,$optionValues);
                ...
                if( in_array ($value,$customValue) ) {
                    if( count($customValue) == 1) {
                        ...
                        $newCustomValue = implode(CRM_CORE_BAO_CUSTOMOPTION_VALUE_SEPERATOR,$customValue);
     ...
     function updateCustomValues($params) 
    {
        require_once 'CRM/Core/BAO/CustomValue.php';
     ...
     * @static
     */
     function getAssoc( $entity_table, $entity_id, &$values ) {
        require_once 'CRM/Core/DAO/CustomOption.php';  
        $dao =& new CRM_Core_DAO_CustomOption( );  
