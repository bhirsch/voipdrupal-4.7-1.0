...
*
*/
$GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_tableName'] =  'civicrm_mobile_provider';
...
$GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_export'] =  null;
...
class CRM_Core_DAO_MobileProvider extends CRM_Core_DAO {
    ...
    *
    * @access public
    * @return civicrm_mobile_provider
    */
        ...
        if (!($GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_import'])) {
            ...
            foreach($fields as $name=>$field) {
                if (CRM_Utils_Array::value('import', $field)) {
                    if ($prefix) {
                        $GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_import']['mobile_provider'] = &$fields[$name];
                    } else {
                        $GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_import'][$name] = &$fields[$name];
                    }
                }
            }
        }
    ...
    */
    ...
    {
        if (!($GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_export'])) {
            $GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_export'] = array();
            $fields = &CRM_Core_DAO_MobileProvider::fields();
            foreach($fields as $name=>$field) {
                if (CRM_Utils_Array::value('export', $field)) {
                    if ($prefix) {
                        $GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_export']['mobile_provider'] = &$fields[$name];
                    } else {
                        $GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_export'][$name] = &$fields[$name];
                    }
                }
            }
        }
        return $GLOBALS['_CRM_CORE_DAO_MOBILEPROVIDER']['_export'];
    }
}
?>