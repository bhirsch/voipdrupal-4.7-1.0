...
$GLOBALS['_CRM_UF_PAGE_FIELD']['_actionLinks'] = null;
...
class CRM_UF_Page_Field extends CRM_Core_Page {
    ...
    {
        if (!isset($GLOBALS['_CRM_UF_PAGE_FIELD']['_actionLinks'])) {
            // helper variable for nicer formatting
            $disableExtra = ts('Are you sure you want to disable this CiviCRM Profile field?');
            $GLOBALS['_CRM_UF_PAGE_FIELD']['_actionLinks'] = array(
        ...
        return $GLOBALS['_CRM_UF_PAGE_FIELD']['_actionLinks'];
    }
    ...
    function browse()
    {
        ...
        while ($ufFieldBAO->fetch()) {
            ...
            $ufField[$ufFieldBAO->id]['action'] = CRM_Core_Action::formLink(CRM_UF_Page_Field::actionLinks(), $action, 
    ...
    {
        $controller =& new CRM_Core_Controller_Simple('CRM_UF_Form_Preview', ts('Preview Custom Data'), CRM_CORE_ACTION_PREVIEW);
        $session =& CRM_Core_Session::singleton();
        $session->pushUserContext(CRM_Utils_System::url('civicrm/admin/uf/group/field','reset=1&action=browse&gid=' . $this->_gid));
