...
$GLOBALS['_CRM_IMPORT_PARSER_CONTACT']['indieFields'] =  null;
...
class CRM_Import_Parser_Contact extends CRM_Import_Parser {
    ...
    var $_mapperKeys;
    var $_mapperLocType;
    var $_mapperPhoneType;
    var $_mapperRelated;
    var $_mapperRelatedContactType;
    var $_mapperRelatedContactDetails;
    var $_mapperRelatedContactEmailType;

    var $_emailIndex;
    var $_firstNameIndex;
    var $_lastNameIndex;

    var $_householdNameIndex;
    var $_organizationNameIndex;
     ...
     * @array
     */
    var $_newRelatedContacts;

    /**
     * class constructor
     */
    function CRM_Import_Parser_Contact( &$mapperKeys, $mapperLocType = null, 
    ...
    function import( $onDuplicate, &$values) {
        ...
        {   
            ...
            if ($code == CRM_CORE_ERROR_DUPLICATE_CONTACT) {
                ...
                } else if ($onDuplicate == CRM_IMPORT_PARSER_DUPLICATE_UPDATE) {
                    $newContact = crm_update_contact_formatted($contactId, $formatted, true);

                } else if ($onDuplicate == CRM_IMPORT_PARSER_DUPLICATE_FILL) {
                    ...
                    if ( ! empty( $value )) {
                        foreach($value as $stateValue ) {
                            if ( $stateValue['state_province']) {
                                if( CRM_Import_Parser_Contact::in_value($stateValue['state_province'],CRM_Core_PseudoConstant::stateProvinceAbbreviation()) 
                                    || CRM_Import_Parser_Contact::in_value($stateValue['state_province'], CRM_Core_PseudoConstant::stateProvince())) {
                                    continue;
                                } else {
                    ...
                    if (!empty( $value ) ) {
                        foreach($value as $stateValue ) {
                            if ( $stateValue['country'] ) {
                                if(CRM_Import_Parser_Contact::in_value($stateValue['country'], CRM_Core_PseudoConstant::countryIsoCode())
                                   || CRM_Import_Parser_Contact::in_value($stateValue['country'], CRM_Core_PseudoConstant::country())) {
                                    continue;
                                } else {
