...
define( 'CRM_UTILS_PAYMENT_PAYPAL_CHARSET','iso-8859-1');
$GLOBALS['_CRM_UTILS_PAYMENT_PAYPAL']['_singleton'] =  null;
...
class CRM_Utils_Payment_PayPal extends CRM_Utils_Payment {
    ...
    function CRM_Utils_Payment_PayPal( $mode ) {
        ...
        $this->_handler =& ProfileHandler_File::getInstance( array(
                                                                   ...
                                                                   'charset' => CRM_UTILS_PAYMENT_PAYPAL_CHARSET,
                                                                   )
                                                             );
     ...
     */ 
     function &singleton( $mode ) {
        if ($GLOBALS['_CRM_UTILS_PAYMENT_PAYPAL']['_singleton'] === null ) { 
            $GLOBALS['_CRM_UTILS_PAYMENT_PAYPAL']['_singleton'] =& new CRM_Utils_Payment_Paypal( $mode );
        } 
        return $GLOBALS['_CRM_UTILS_PAYMENT_PAYPAL']['_singleton']; 
    } 
    ...
    function setExpressCheckOut( &$params ) {
        ...
        if ( Services_PayPal::isError( $setExpressCheckoutRequestDetails ) ) {
            return CRM_Utils_Payment_PayPal::error( $setExpressCheckoutRequestDetails );
        }

        $setExpressCheckoutRequestDetails->setCancelURL ( $params['cancelURL'], CRM_UTILS_PAYMENT_PAYPAL_CHARSET);
        $setExpressCheckoutRequestDetails->setReturnURL ( $params['returnURL'], CRM_UTILS_PAYMENT_PAYPAL_CHARSET);
        $setExpressCheckoutRequestDetails->setInvoiceID ( $params['invoiceID'], CRM_UTILS_PAYMENT_PAYPAL_CHARSET);
        ...
        if ( Services_PayPal::isError( $setExpressCheckout ) ) {
            return CRM_Utils_Payment_PayPal::error( $setExpressCheckout );
        }
    ...
    function getExpressCheckoutDetails( $token ) {
        ...
        if ( Services_PayPal::isError( $getExpressCheckoutDetails ) ) {
            return CRM_Utils_Payment_PayPal::error( $getExpressCheckoutDetails );
        }

        $getExpressCheckoutDetails->setToken( $token, CRM_UTILS_PAYMENT_PAYPAL_CHARSET);
    ...
    function doExpressCheckout( &$params ) {
        ...
        if ( Services_PayPal::isError( $doExpressCheckoutPaymentRequestDetails ) ) {
            return CRM_Utils_Payment_PayPal::error( $doExpressCheckoutPaymentRequestDetails );
        }

        $doExpressCheckoutPaymentRequestDetails->setPaymentDetails( $paymentDetails );
        $doExpressCheckoutPaymentRequestDetails->setPayerID       ( $params['payer_id']      , CRM_UTILS_PAYMENT_PAYPAL_CHARSET);
        $doExpressCheckoutPaymentRequestDetails->setToken         ( $params['token']         , CRM_UTILS_PAYMENT_PAYPAL_CHARSET);
        $doExpressCheckoutPaymentRequestDetails->setPaymentAction ( $params['payment_action'], CRM_UTILS_PAYMENT_PAYPAL_CHARSET);
        ...
        if ( Services_PayPal::isError( $doExpressCheckoutPayment ) ) {
            return CRM_Utils_Payment_PayPal::error( $doExpressCheckoutPayment );
        }
        ...
        $params['gross_amount'  ] = CRM_Utils_Payment_PayPal::getAmount( $paymentInfo->GrossAmount );
        $params['fee_amount'    ] = CRM_Utils_Payment_PayPal::getAmount( $paymentInfo->FeeAmount    );
        $params['net_amount'    ] = CRM_Utils_Payment_PayPal::getAmount( $paymentInfo->SettleAmount );
