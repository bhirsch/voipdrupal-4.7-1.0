...
define( 'CRM_UTILS_PAYMENT_MONERIS_CHARSET','UFT-8');
$GLOBALS['_CRM_UTILS_PAYMENT_MONERIS']['_singleton'] =  null;
...
class CRM_Utils_Payment_Moneris extends CRM_Utils_Payment { 
     ...
     * @return void 
     */ 
    function CRM_Utils_Payment_Moneris( $mode ) {
        ...
        if ('CAD' != $config->currencyID) {
          return CRM_Utils_Payment_Moneris::error(); // Configuration error: default currency must be CAD
        }
    }
     ...
     */ 
     function &singleton( $mode ) {
        if ($GLOBALS['_CRM_UTILS_PAYMENT_MONERIS']['_singleton'] === null ) { 
            $GLOBALS['_CRM_UTILS_PAYMENT_MONERIS']['_singleton'] =& new CRM_Utils_Payment_Moneris( $mode );
        } 
        return $GLOBALS['_CRM_UTILS_PAYMENT_MONERIS']['_singleton']; 
    } 
    ...
    function doDirectPayment( &$params ) {
      ...
      if ( ! $this->_profile ) {
          return CRM_Utils_Payment_Moneris::error( );
      }
      ...
      if ( CRM_Utils_Payment_Moneris::isError( $mpgResponse ) ) {
          return CRM_Utils_Payment_Moneris::error( $mpgResponse );
      }

      /* Check for application errors */
      $result =& CRM_Utils_Payment_Moneris::checkResult( $mpgResponse );
