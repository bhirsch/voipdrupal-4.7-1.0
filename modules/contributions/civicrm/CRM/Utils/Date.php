 ...
 * Date utilties
 */
$GLOBALS['_CRM_UTILS_DATE']['abbrWeekdayNames'] = null;
$GLOBALS['_CRM_UTILS_DATE']['fullWeekdayNames'] = null;
$GLOBALS['_CRM_UTILS_DATE']['abbrMonthNames'] = null;
$GLOBALS['_CRM_UTILS_DATE']['fullMonthNames'] = null;
$GLOBALS['_CRM_UTILS_DATE']['replacements'] =  array(
            '%b' => 'M',
            '%B' => 'F',
            '%d' => 'd',
            '%e' => 'j',
...
class CRM_Utils_Date {
     ...
     function &getAbbrWeekdayNames()
    {
        ...
        if (!isset($GLOBALS['_CRM_UTILS_DATE']['abbrWeekdayNames'])) {

            // set LC_TIME and build the arrays from locale-provided names
            // June 1st, 1970 was a Monday
            CRM_Core_I18n::setLcTime();
            for ($i = 0; $i < 7; $i++) {
                $GLOBALS['_CRM_UTILS_DATE']['abbrWeekdayNames'][$i] = strftime('%a', mktime(0, 0, 0, 6, $i, 1970));
            }
        }
        return $GLOBALS['_CRM_UTILS_DATE']['abbrWeekdayNames'];
    }
     ...
     function &getFullWeekdayNames()
    {
        ...
        if (!isset($GLOBALS['_CRM_UTILS_DATE']['fullWeekdayNames'])) {

            // set LC_TIME and build the arrays from locale-provided names
            // June 1st, 1970 was a Monday
            CRM_Core_I18n::setLcTime();
            for ($i = 0; $i < 7; $i++) {
                $GLOBALS['_CRM_UTILS_DATE']['fullWeekdayNames'][$i] = strftime('%A', mktime(0, 0, 0, 6, $i, 1970));
            }
        }
        return $GLOBALS['_CRM_UTILS_DATE']['fullWeekdayNames'];
    }
     ...
     * @static
     */
     function &getAbbrMonthNames()
    {
        
        if (!isset($GLOBALS['_CRM_UTILS_DATE']['abbrMonthNames'])) {

            // set LC_TIME and build the arrays from locale-provided names
            CRM_Core_I18n::setLcTime();
            for ($i = 1; $i <= 12; $i++) {
                $GLOBALS['_CRM_UTILS_DATE']['abbrMonthNames'][$i] = strftime('%b', mktime(0, 0, 0, $i, 10, 1970 ));
            }
        }
        return $GLOBALS['_CRM_UTILS_DATE']['abbrMonthNames'];
    }
     ...
     * @static
     */
     function customFormat($dateString, $format = null)
    {
     ...
     * @static
     */
     function posixToPhp($format)
    {
        
        return strtr($format, $GLOBALS['_CRM_UTILS_DATE']['replacements']);
    }
